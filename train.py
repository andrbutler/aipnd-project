...
def get_args():
    ...
    parser.add_argument('data_dir', type = str, help = 'path to the data to train the network with')

    parser.add_argument('--save_dir', type = str, help = 'path to save checkpoint of trained neural network.', default = 'checkpoints')

    parser.add_argument('--gpu', help = 'enables gpu mode', action = 'store_true')

    parser.add_argument('--arch', help = 'sets pretrained model architecture', type = str, default = 'vgg')

    parser.add_argument('--epochs', help = 'sets amount of epochs to train the network', type = int, default = 5)

    parser.add_argument('--learn_rate', help = 'sets the learning rate used when training the network', type = int, default = 0.01)

    parser.add_argument('--hidden_units', help = 'sets amount of hidden units in the network', type = int, default = 512)
...
def set_loaders(data):
    ...
    train_dataset = datasets.ImageFolder(data['train'], data_transforms_train)
    valid_dataset = datasets.ImageFolder(data['valid'], data_transforms_valid)
    test_dataset = datasets.ImageFolder(data['test'], data_transforms_test)

    trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=24)
    validloader = torch.utils.data.DataLoader(valid_dataset, batch_size=24)
    testloader = torch.utils.data.DataLoader(test_dataset, batch_size=24)
    
    loaders = {'train': trainloader, 'valid': validloader, 'test': testloader, 'trainset': train_dataset}
...
def train(model, device, epochs, trainloader, validloader, learn_rate):
    ...
    for i in range(epochs):
        ...
        for ii, (inputs, labels) in enumerate(trainloader):
            ...
            if steps % print_every == 0:
                ...
                with torch.no_grad():
                    valid_loss, accuracy = validation(model, validloader, device)

                print("Epoch: {}/{}... ".format(i+1, epochs),
                      "Training Loss: {:.3f}.. ".format(running_loss/print_every),
                      "validation Loss: {:.3f}.. ".format(valid_loss/len(validloader)),
                      "validation Accuracy: {:.3f}".format(accuracy/len(validloader)), flush = True)
