...
def get_args():
    ...
    parser.add_argument('--topk', help = 'sets number of results to return from most to least likely', type = int, default = 5)

    parser.add_argument('--category_names', help = 'sets path to dictionay of names to display in output', type = str, default = ' ')
    ...
    if args.category_names != ' ':
        ...
        for result in range(len(probs)):
            print('rank: {:<3}, name: {}, class: {:<3}, probability: {:.4f}\n'.format(result + 1,
                    names[result], classes[result], probs[result]))
